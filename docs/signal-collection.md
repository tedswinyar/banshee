...
| Jetsam kills | `sysctl -n kern.memorystatus.kill_on_sustained_pressure_count` | Lifetime counter of processes the kernel killed under sustained memory pressure — cheap integer read, 0 on a healthy machine, increments on a kill. The `Jetsam` dimension since schema v13; a positive DELTA over the window is a confirmed kill, **not** advisory, and rings the top bell AT ONCE (no sustain grace — the kernel already destroyed a process). Delta invalidated across a `boot_time_secs` change (a reboot resets the counter). Stored per sample; NULL marks a pre-v13 row. |
...
- **Swap %** = `used ÷ total`. Yellow `≥ 60`, red `≥ 90`.

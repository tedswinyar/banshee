...
| The release succeeds, then the NEXT push fails verify with hundreds of `sbom/*.cdx.json: … matches [e-mail pattern]` findings | `release.sh` runs verify BEFORE it generates the notices and SBOM, and the job pushes main `--no-verify`, so the attribution commit was never gated; the files carry third-party authors' addresses by nature (first end-to-end release, v0.1.5, 2026-09-20) | fixed: the gate masks e-mail addresses in exactly those two generated files (home paths, LAN addresses and the denylist still apply to them) and `release.sh` re-runs the gate after the attribution commit, before tagging |
